...
static tA2DP_STATUS A2DP_ParseInfoLhdcV3Sink(tA2DP_LHDCV3_SINK_CIE* p_ie,
  ...
  if (p_ie == NULL || p_codec_info == NULL) return A2DP_FAIL;

  // Check the codec capability length
  losc = *p_codec_info++;

  if (losc != A2DP_LHDCV3_CODEC_LEN) return A2DP_FAIL;
...
bool A2DP_IsVendorSinkCodecValidLhdcV3([[maybe_unused]] const uint8_t* p_codec_info) {
  return false;
...
static tA2DP_STATUS A2DP_CodecInfoMatchesCapabilityLhdcV3Sink(
  ...
  /* bit per sample */
  if ((cfg_cie.bits_per_sample & p_cap->bits_per_sample) == 0) return A2DP_FAIL;
...
bool A2dpCodecConfigLhdcV3Sink::init() {
  ...
  // Load the decoder
  log::info("LHDC V3: Sink role not supported - decoder not available");
  return false;
}
...
tA2DP_STATUS A2dpCodecConfigLhdcV3Base::setCodecConfig(const uint8_t* p_peer_codec_info, bool is_capability,
  ...
  if (!is_source_) {
      log::info("LHDC V5: Cannot configure sink - decoder not available");
      goto fail;
    }
  if (status != A2DP_SUCCESS) {
